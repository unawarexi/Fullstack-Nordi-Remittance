...
const sizeStyles: Record<
  InputSize,
  { input: string; label: string; helper: string; icon: string }
> = {
  sm: {
...
const variantStyles: Record<InputVariant, { base: string; focus: string }> = {
  ...
  filled: {
    ...
    focus:
      "focus:bg-neutral-50 dark:focus:bg-neutral-700 focus:ring-2 focus:ring-primary-500/20",
  },
  flushed: {
        ...
        <div className="relative">
          ...
          <div className="absolute right-3 top-1/2 flex -translate-y-1/2 items-center gap-2">
            ...
            {hasSuccess && !isPassword && (
              <CheckCircle
                className={cn("text-success-500", sizeConfig.icon)}
              />
            )}
            ...
            {isPassword && showPasswordToggle && (
              <button
                ...
                className="text-neutral-400 transition-colors hover:text-neutral-600 focus:outline-none dark:text-neutral-500 dark:hover:text-neutral-300"
                tabIndex={-1}
              >
                {showPassword ? (
