...
export const KycRepository = {
  ...
  submitPersonalInfo: async (data: PersonalInfoRequest): Promise<ApiResponse<{ message: string }>> => {
    const response = await apiClient.post<ApiResponse<{ message: string }>>(`/kyc/personal-info`, data);
    return response.data;
  },
  ...
  submitAddress: async (data: AddressInfoRequest): Promise<ApiResponse<{ message: string }>> => {
    const response = await apiClient.post<ApiResponse<{ message: string }>>(`/kyc/address`, data);
    return response.data;
  },
  ...
  getAddress: async (): Promise<ApiResponse<AddressInfoRequest & { status: string }>> => {
    const response = await apiClient.get<ApiResponse<AddressInfoRequest & { status: string }>>(`/kyc/address`);
    return response.data;
  },
  ...
  submitEmployment: async (data: EmploymentInfoRequest): Promise<ApiResponse<{ message: string }>> => {
    const response = await apiClient.post<ApiResponse<{ message: string }>>(`/kyc/employment`, data);
    return response.data;
  },
  ...
  getDocuments: async (): Promise<ApiResponse<KycDocument[]>> => {
    const response = await apiClient.get<ApiResponse<KycDocument[]>>(ApiEndpoints.kycDocuments);
    return response.data;
  },
   ...
   */
  getDocumentById: async (documentId: UUID): Promise<ApiResponse<KycDocument>> => {
    const response = await apiClient.get<ApiResponse<KycDocument>>(ApiEndpoints.kycDocument(documentId));
    return response.data;
  },
  ...
  uploadDocument: async (data: {
    ...
    const response = await apiClient.post<ApiResponse<KycDocument>>(`/kyc/documents/identity`, formData, {
      headers: {
  ...
  uploadProofOfAddress: async (data: {
    ...
    const response = await apiClient.post<ApiResponse<KycDocument>>(`/kyc/documents/address`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  ...
  uploadSelfie: async (selfie: File): Promise<ApiResponse<KycDocument>> => {
    ...
    const response = await apiClient.post<ApiResponse<KycDocument>>(`/kyc/documents/selfie`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  ...
  deleteDocument: async (documentId: UUID): Promise<ApiResponse<{ message: string }>> => {
    const response = await apiClient.delete<ApiResponse<{ message: string }>>(ApiEndpoints.kycDocument(documentId));
    return response.data;
  },
   ...
   */
  startVerification: async (): Promise<
    ApiResponse<{
      verificationId: UUID;
      status: string;
      message: string;
  ...
  > => {
    ...
    >(ApiEndpoints.kycVerifyStatus(verificationId));
    return response.data;
  },
  ...
  requestReverification: async (reason?: string): Promise<ApiResponse<{ message: string }>> => {
    const response = await apiClient.post<ApiResponse<{ message: string }>>(`/kyc/reverify`, { reason });
    return response.data;
  },
   ...
   */
  getVerificationHistory: async (): Promise<ApiResponse<any[]>> => {
    const response = await apiClient.get<ApiResponse<any[]>>(`/kyc/history`);
    return response.data;
  },
  ...
  getAdminPendingReviews: async (params?: { page?: number; limit?: number; status?: string }) => {
    const response = await apiClient.get(ApiEndpoints.kycAdminPending, { params });
    return response.data;
  },

  getAdminUserKyc: async (userId: string) => {
    const response = await apiClient.get(ApiEndpoints.kycAdminUser(userId));
    return response.data;
  },

  adminReviewKyc: async (userId: string, data: { status: "approved" | "rejected" | "pending"; notes?: string }) => {
    const response = await apiClient.patch(ApiEndpoints.kycAdminReview(userId), data);
    return response.data;
  },

  getAdminKycStats: async () => {
    const response = await apiClient.get(ApiEndpoints.kycAdminStats);
    return response.data;
  },
};
