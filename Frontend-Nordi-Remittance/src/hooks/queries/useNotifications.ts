...
import { notificationsApi } from "../../core/api/endpoints/notifications.api";
...
export const useUnreadNotifications = (limit?: number) => {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await notificationsApi.getUnread(limit);
      return response.data;
    },
  });
};
...
export const useUnregisterPushToken = () => {
  ...
  return useMutation({
    mutationFn: async (deviceId: string) => {
      const response = await notificationsApi.unregisterPushToken(deviceId);
      return response.data;
    },
...
export const useSendTestNotification = () => {
  ...
  return useMutation({
    mutationFn: async () => {
      const response = await notificationsApi.testPushNotification();
      return response.data;
    },
