...
import { useAdminUserFinancialData } from "@hooks/api-queries";
...
function mapTransaction(t: any): Transaction {
  ...
  return {
    ...
    status: t.status === "completed" || t.status === "pending" || t.status === "failed" ? t.status : "completed",
    reference: t.referenceNumber || "",
  };
}

function mapCard(c: any, transactions: any[]): CardDetails {
  const cardTransactions = transactions.filter((t: any) => t.category === "cards").map(mapTransaction);

  const expiryDate =
    c.expiryMonth && c.expiryYear ? `${String(c.expiryMonth).padStart(2, "0")}/${String(c.expiryYear).slice(-2)}` : "";
