...
import { useAdminPendingReviews, useAdminKycStats, useAdminReviewKyc } from "@hooks/api-queries";
...
export function useKycPendingUsers() {
  ...
  const approveUser = useCallback(
    (userId: string) => {
      reviewMutation.mutate({ userId, data: { status: "approved" } }, { onSuccess: () => refetch() });
    },
  ...
  const rejectUser = useCallback(
    (userId: string, reason: string) => {
      reviewMutation.mutate({ userId, data: { status: "rejected", notes: reason } }, { onSuccess: () => refetch() });
    },
    [reviewMutation, refetch],
  );
