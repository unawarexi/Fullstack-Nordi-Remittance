...
import { useInvestments, useInvestmentPortfolio, useInvestmentProducts } from "@hooks/api-queries";
...
export function useInvestmentsManagement() {
  ...
  const rawInvestments = useMemo(() => {
    ...
    return raw.map((inv: any) => ({
      ...
      investor:
        inv.userName ||
        (inv.user?.firstName && inv.user?.lastName ? `${inv.user.firstName} ${inv.user.lastName}` : "Investor"),
  ...
  const products = useMemo(() => {
    ...
    const raw: any[] = Array.isArray(outer) ? outer : Array.isArray(outer?.data) ? outer.data : outer?.products || [];
  ...
  const filtered = useMemo(() => {
    ...
    return multiKeySort(result, [{ getter: (i: any) => new Date(i.startDate || 0), direction: "desc" }]);
  ...
  return {
    ...
    setStatusFilter: useCallback((v: InvestmentStatusFilter) => {
      setStatusFilter(v);
      setPage(1);
    }, []),
    setTypeFilter: useCallback((v: InvestmentTypeFilter) => {
      setTypeFilter(v);
      setPage(1);
    }, []),
    setPage,
