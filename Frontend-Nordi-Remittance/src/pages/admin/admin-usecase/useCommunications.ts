...
export function useCommunications() {
    ...
    return [
      {
        id: "ET-001",
        name: "Welcome Email",
        subject: "Welcome to Nordi Remittance!",
        category: "onboarding",
        status: "active",
        lastEdited: "",
        sentCount: 0,
      ...
      },
      {
        ...
        subject: "Special rates on international transfers!",
        category: "marketing",
        status: "draft",
        lastEdited: "",
        sentCount: 0,
    ...
    return [
      {
        ...
        message: "Your Nordi verification code is {{code}}. Expires in 5 min.",
        category: "security",
        status: "active",
        sentCount: 0,
      },
      {
        id: "SMS-002",
        name: "Transfer Alert",
        message: "{{amount}} sent to {{recipient}}. Ref: {{ref}}",
        category: "transaction",
        status: "active",
        sentCount: 0,
      },
      {
        id: "SMS-003",
        name: "Login Alert",
        message: "New login detected on your Nordi account from {{device}}.",
        category: "security",
        status: "active",
        sentCount: 0,
      },
      {
        ...
        message: "{{amount}} deposited to your wallet. Balance: {{balance}}",
        category: "transaction",
        status: "active",
        sentCount: 0,
      },
    ...
    return [
      {
        ...
        body: "Your transfer of {{amount}} to {{name}} is complete.",
        status: "active",
        sentCount: 0,
      },
      {
      ...
      },
      {
        id: "PUSH-003",
        name: "Promo Rate",
        title: "Special Exchange Rate!",
        body: "Send money to {{country}} at just {{rate}} today.",
        status: "draft",
        sentCount: 0,
      },
    ];
  ...
  const campaigns = useMemo(() => {
    ...
    return [
      ...
      {
        id: "CMP-003",
        name: "Feature Announcement - Cards",
        type: "push",
        status: "scheduled",
        recipients: 0,
        opened: 0,
  ...
  const filtered = useMemo(() => {
    ...
    return multiKeySort(result, [{ getter: (i: any) => new Date(i.lastEdited || i.sentDate || 0), direction: "desc" }]);
  ...
  return {
    ...
    pageNumbers,
    setSearch: useCallback((v: string) => {
      setSearch(v);
      setPage(1);
    }, []),
    setCategoryFilter: useCallback((v: CategoryFilter) => {
      setCategoryFilter(v);
      setPage(1);
    }, []),
    setActiveSection,
