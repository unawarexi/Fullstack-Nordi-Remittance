...
import { useSuspiciousTransactionManagement } from "../../domain/useTransactionManagement";
...
export default function SuspiciousTransactions() {
  ...
  return (
    <PageContainer>
      ...
      <StatsGrid>
        <StatCard label="Suspicious Transactions" value={total} icon={<ShieldAlert size={18} />} iconColor="from-rose-500 to-rose-600" change={`${fraudSignals.length} fraud signals`} positive={false} index={0} />
      </StatsGrid>

      <FilterBar searchValue={search} onSearchChange={setSearch} searchPlaceholder="Search suspicious transactions..." />
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          {signal ? (
                            <div>
                              <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 font-medium">
                                <AlertTriangle size={10} />
                                {signal.type ?? signal.severity ?? "Flagged"}
                              </span>
                              {signal.description && (
                                <p className="text-[10px] text-gray-400 mt-0.5 max-w-[200px] truncate">{signal.description}</p>
                              )}
                            </div>
                          ) : (
                            <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400">
                              Suspicious status
                            </span>
                          )}
                        </td>
