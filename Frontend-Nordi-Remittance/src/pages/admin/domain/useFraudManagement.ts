...
export function useFraudManagement() {
  ...
  const dismissAlert = useCallback(
    (signalId: string, callbacks?: { onSuccess?: () => void }) => {
      updateSignal.mutate(
        ...
        { onSuccess: () => { refetchSignals(); callbacks?.onSuccess?.(); } },
      );
    },
  ...
  const resolveCase = useCallback(
    (caseId: string, resolution: string, callbacks?: { onSuccess?: () => void }) => {
      updateCase.mutate(
        ...
        { onSuccess: () => { refetchCases(); callbacks?.onSuccess?.(); } },
      );
    },
  ...
  return {
    ...
    setSeverityFilter: useCallback((v: FraudSeverityFilter) => { setSeverityFilter(v); setPage(1); }, []),
    setStatusFilter: useCallback((v: FraudStatusFilter) => { setStatusFilter(v); setPage(1); }, []),
    setActiveTab,
    setPage,
    setSelectedAlertId,
    dismissAlert,
