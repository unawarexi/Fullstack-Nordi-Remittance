...
export function useKycManagement() {
  ...
  const { data: usersRaw } = useSearchUsers({ limit: 500, kycStatus: statusFilter !== "all" ? statusFilter : undefined });
  ...
  const filtered = useMemo(() => {
    ...
    return multiKeySort(result, [
      { getter: (u: any) => new Date(u.submittedAt || 0), direction: "desc" },
    ]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalApproved: s.totalApproved ?? s.approved ?? rawApplications.filter((u: any) => u.kycStatus === "approved").length,
      totalRejected: s.totalRejected ?? s.rejected ?? rawApplications.filter((u: any) => u.kycStatus === "rejected").length,
  ...
  const approveUser = useCallback(
    (userId: string, callbacks?: { onSuccess?: () => void }) => {
      reviewMutation.mutate(
        ...
        { onSuccess: () => { refetch(); callbacks?.onSuccess?.(); setReviewModal(null); } },
      );
    },
  ...
  return {
    ...
    setStatusFilter: useCallback((v: KycStatusFilter) => { setStatusFilter(v); setPage(1); }, []),
    setPage,
    setSelectedUserId,
    setReviewModal,
    approveUser,
