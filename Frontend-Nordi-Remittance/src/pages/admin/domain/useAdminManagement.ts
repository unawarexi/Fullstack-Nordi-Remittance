...
export function useAdminManagement() {
  ...
  const { data: adminsRaw, isLoading: adminsLoading, refetch: refetchAdmins } = useAdminUsersList({ page: 1, limit: 500 });
  ...
  const filtered = useMemo(() => {
    ...
    return multiKeySort(result, [
      { getter: (i: any) => new Date(i.createdAt || i.timestamp || 0), direction: "desc" },
    ]);
  ...
  const handleCreateAdmin = useCallback(
    (data: any, callbacks?: { onSuccess?: () => void; onError?: (err: any) => void }) => {
      createAdmin.mutate(data, {
        onSuccess: () => { refetchAdmins(); callbacks?.onSuccess?.(); },
  ...
  return {
    ...
    setRoleFilter: useCallback((v: AdminRoleFilter) => { setRoleFilter(v); setPage(1); }, []),
    setActiveTab,
    setPage,
    handleCreateAdmin,
    refetch,
