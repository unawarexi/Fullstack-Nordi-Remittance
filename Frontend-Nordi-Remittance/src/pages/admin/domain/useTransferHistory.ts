...
import { useRecentTransactions } from "@hooks/queries";
import {
  applyFilterPipeline,
  textSearchFilter,
  enumFilter,
...
export function useTransferHistory() {
  ...
  const filtered = useMemo(() => {
    ...
    return multiKeySort(result, [
      { getter: (i: any) => new Date(i.date || 0), direction: "desc" },
    ]);
  ...
  return {
    ...
    pageNumbers,
    setStatusFilter: useCallback((v: StatusFilter) => { setStatusFilter(v); setPage(1); }, []),
    setTimeFilter: useCallback((v: TimeFilter) => { setTimeFilter(v); setPage(1); }, []),
    setPage,
    refetch,
  };
}
