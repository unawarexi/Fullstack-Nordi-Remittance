...
export function useTransactionManagement() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (filters.timeRange !== "all") {
      ...
      if (from || to) {
        predicates.push(
          dateRangeFilter((tx) => tx.createdAt ?? tx.date, from, to),
        );
      }
    }
    ...
    if (filters.minAmount !== undefined || filters.maxAmount !== undefined) {
      predicates.push(
        numericRangeFilter((tx) => tx.amount, filters.minAmount, filters.maxAmount),
      );
    }
    ...
    return multiKeySort(filtered, [
      {
        getter: (tx: any) =>
          filters.sortBy === "amount"
            ? tx.amount ?? 0
            : new Date(tx.createdAt ?? tx.date ?? 0),
  ...
  const updateFilter = useCallback(
    (key: keyof TransactionFiltersState, value: any) => {
...
export function useFailedTransactionManagement() {
  ...
  const sorted = useMemo(
    () =>
      multiKeySort(filtered, [
        { getter: (tx: any) => new Date(tx.createdAt ?? tx.date ?? 0), direction: "desc" },
      ]),
    [filtered],
  );
