...
import { useSpendingByCategory, useSpendingTrends } from "@hooks/queries";
import { multiKeySort } from "@core/algo";
import { PERIOD_MAP, CHART_COLORS } from "./constants/dashboard.constants";
...
export function useClientSpending(): ClientSpendingData {
  ...
  const categories = useMemo<SpendingCategory[]>(() => {
    ...
    return multiKeySort(mapped, [
      { getter: (c) => c.amount, direction: "desc" },
    ]);
  }, [catRes]);
  ...
  const trends = useMemo<SpendingTrendPoint[]>(() => {
    ...
    return multiKeySort(mapped, [
      { getter: (t) => t.month, direction: "asc" },
    ]);
  }, [trendRes]);

  const totalSpending = useMemo(
    () => categories.reduce((s, item) => s + item.amount, 0),
    [categories],
  );
