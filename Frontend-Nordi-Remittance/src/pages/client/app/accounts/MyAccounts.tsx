...
import { useClientWallets, useClientAccountSummary } from "../../client-usecase/useaccounts-client-usecase";
...
const MyAccounts: React.FC = () => {
  ...
  const filteredWallets =
    selectedType === "all"
      ? wallets
      : wallets.filter(
          (w: any) => w.type?.toLowerCase() === selectedType || w.accountType?.toLowerCase() === selectedType,
        );
    ...
    <PageContainer>
      ...
      {isLoading ? (
        ...
        <StatsGrid cols={4}>
          <StatCard
            label="Total Balance"
            value={showBalances ? formatCurrency(summary?.totalBalance || 0) : "••••••"}
            icon={<Wallet size={20} />}
          ...
          <StatCard
            label="Income (30d)"
            value={showBalances ? formatCurrency(summary?.totalIncome || 0) : "••••••"}
          ...
          <StatCard
            label="Expenses (30d)"
            value={showBalances ? formatCurrency(summary?.totalExpenses || 0) : "••••••"}
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`h-1.5 bg-gradient-to-r ${accountTypeColors[type] || "from-indigo-500 to-purple-600"}`}
                  />
                  <div className="p-4 sm:p-5" onClick={() => navigate(`/customer/accounts/${type}`)}>
                    <div className="mb-3 flex items-center justify-between sm:mb-4">
                      <div className="flex items-center gap-2.5 sm:gap-3">
                        <div
                          className={`rounded-xl bg-gradient-to-br p-2 sm:p-2.5 ${accountTypeColors[type] || "from-indigo-500 to-purple-600"} text-white`}
                        >
                    ...
                    <div className="mb-3 sm:mb-4">
                      ...
                      <p className="text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">
                        {showBalances
                          ? formatCurrency(wallet.balance || wallet.availableBalance || 0, wallet.currency || "USD")
                          : "••••••"}
                      </p>
                    </div>
