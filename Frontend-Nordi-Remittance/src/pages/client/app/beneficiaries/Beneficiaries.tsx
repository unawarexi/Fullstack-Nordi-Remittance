    ...
    <PageContainer>
      ...
      <motion.div variants={dashboardItemVariants}>
        <PageHeader
          ...
          breadcrumbs={[{ label: "Dashboard", href: "/customer/dashboard" }, { label: "Beneficiaries" }]}
          actions={
            <ActionButton
      ...
      {isLoading ? (
        ...
        <motion.div className="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filteredBeneficiaries.map((b: any, index: number) => (
            <motion.div
              ...
              animate="visible"
            >
              <DashCard className="transition-all hover:border-indigo-300 dark:hover:border-indigo-700" hover>
                <div className="mb-3 flex items-start justify-between">
                  <div className="flex items-center gap-2.5 sm:gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 text-xs font-semibold text-white sm:h-10 sm:w-10 sm:text-sm">
                      {(b.name || "U").charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <h4 className="text-xs font-semibold text-gray-900 dark:text-white sm:text-sm">
                        {b.name || "Unknown"}
                      </h4>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button className="rounded p-1 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800">
                      <MoreVertical size={14} className="text-gray-400 dark:text-gray-500" />
                    </button>
                  </div>
                </div>

                <div className="mb-3 space-y-1 sm:mb-4 sm:space-y-1.5">
                  {b.accountNumber && (
                    <p className="flex items-center gap-2 text-[10px] text-gray-500 dark:text-gray-400 sm:text-xs">
                      <Building2 size={12} />
                ...
                <div className="flex gap-2">
                  ...
                  <motion.button
                    className="flex items-center justify-center gap-1.5 rounded-lg bg-gray-50 px-2.5 py-1.5 text-[10px] font-medium text-gray-600 transition-colors hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 sm:px-3 sm:py-2 sm:text-xs"
                    whileTap={{ scale: 0.95 }}
                  >
                    <Edit3 size={12} />
                  </motion.button>
                  <motion.button
                    className="flex items-center justify-center gap-1.5 rounded-lg bg-rose-50 px-2.5 py-1.5 text-[10px] font-medium text-rose-600 transition-colors hover:bg-rose-100 dark:bg-rose-950/50 dark:text-rose-400 dark:hover:bg-rose-950/80 sm:px-3 sm:py-2 sm:text-xs"
                    whileTap={{ scale: 0.95 }}
