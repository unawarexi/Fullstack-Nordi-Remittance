...
const Transactions: React.FC = () => {
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter((t: any) => {
      const matchSearch = !searchQuery || t.description?.toLowerCase().includes(searchQuery.toLowerCase()) || t.reference?.toLowerCase().includes(searchQuery.toLowerCase()) || t.recipientName?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <PageContainer>
        ...
        <PageHeader
          ...
          actions={
            <div className="flex gap-2 sm:gap-3">
              <ActionButton label="Export" icon={<Download size={16} />} variant="secondary" onClick={() => navigate("/customer/transactions/download")} />
              <ActionButton label="New Transfer" icon={<Send size={16} />} onClick={() => navigate("/customer/send/domestic")} />
            </div>
          }
        />
      </motion.div>
      ...
      {isLoading ? (
        ...
        <StatsGrid cols={4}>
          <StatCard label="Total Transactions" value={stats.total} icon={<Repeat size={20} />} iconColor="from-indigo-500 to-purple-500" index={0} />
          <StatCard label="Total Volume" value={showBalances ? formatCurrency(stats.totalAmount) : "••••••"} icon={<BarChart3 size={20} />} iconColor="from-emerald-500 to-teal-500" index={1} />
          <StatCard label="Completed" value={stats.completed} icon={<CheckCircle2 size={20} />} iconColor="from-green-500 to-emerald-500" index={2} />
          <StatCard label="Pending" value={stats.pending} icon={<Clock size={20} />} iconColor="from-amber-500 to-orange-500" index={3} />
        </StatsGrid>
      )}
      ...
      <FilterBar searchValue={searchQuery} onSearchChange={setSearchQuery} searchPlaceholder="Search transactions...">
        ...
        <motion.button
          onClick={() => refetch()}
          className="p-1.5 sm:p-2 rounded-lg bg-gray-100 dark:bg-gray-800 text-indigo-600 dark:text-indigo-400 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
          whileTap={{ scale: 0.95 }}
        >
          <RefreshCw size={16} />
        </motion.button>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description={searchQuery ? "No transactions match your search criteria." : "Your transaction history will appear here once you start making transfers."}
            ...
            {filteredTransactions.slice(0, 20).map((tx: any, index: number) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center gap-3 sm:gap-4 px-3 sm:px-4 py-2.5 sm:py-3.5 hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer transition-colors"
                  custom={index}
                  ...
                  onClick={() => navigate("/customer/transactions/history")}
                >
                  <div className={`p-1.5 sm:p-2 rounded-full bg-gray-100 dark:bg-gray-800 ${tConfig.color}`}>
                    {tConfig.icon}
                  </div>
                  ...
                  <div className="text-right">
                    <p className={`text-xs sm:text-sm font-semibold ${isCredit ? "text-emerald-600 dark:text-emerald-400" : "text-gray-900 dark:text-white"}`}>
                      {showBalances ? `${isCredit ? "+" : "-"}${formatCurrency(tx.amount || 0, tx.currency)}` : "••••••"}
                    </p>
                  </div>
                </motion.div>
              );
      ...
      {/* Quick Links */}
      <QuickLinksGrid>
        <QuickLinkCard label="Recent Activity" icon={<Clock size={20} />} route="/customer/transactions/recent" iconColor="text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/50" />
        <QuickLinkCard label="Scheduled" icon={<Calendar size={20} />} route="/customer/transactions/scheduled" iconColor="text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/50" />
        <QuickLinkCard label="Full History" icon={<BarChart3 size={20} />} route="/customer/transactions/history" iconColor="text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50" />
        <QuickLinkCard label="Download" icon={<Download size={20} />} route="/customer/transactions/download" iconColor="text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/50" />
      </QuickLinksGrid>
    </PageContainer>
  );
};
