      ...
      <Container size="xl">
          ...
          {partners.map((partner, index) => (
            <motion.div
              ...
              className={cn(
                ...
                "hover:text-neutral-600 hover:border-neutral-200 transition-all"
              )}
            >
              {partner.logo}
            </motion.div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {awards.map((award, index) => (
            ...
            >
              <div className="inline-flex p-3 rounded-full bg-amber-50 text-amber-500 mb-3">
                {award.icon}
              </div>
              <h4 className="font-semibold text-neutral-900 text-sm">
                {award.title}
              </h4>
              <p className="text-xs text-neutral-500 mt-1">{award.org}</p>
              <span className="inline-block mt-2 px-2 py-0.5 text-xs font-medium text-amber-600 bg-amber-50 rounded">
                {award.year}
              </span>
            </motion.div>
          ))}
