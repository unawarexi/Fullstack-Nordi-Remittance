...
const AccountCard: React.FC<AccountCardProps> = ({ account, index }) => (
  ...
  >
    ...
    <div className="p-5 pb-4">
      <div className={cn("inline-block w-10 h-1 rounded-full mb-3", account.color)} />
      <h3 className="text-lg font-semibold text-neutral-900 dark:text-neutral-100">{account.name}</h3>
      <p className="text-sm text-neutral-500 dark:text-neutral-400">{account.tagline}</p>
      <div className="mt-4">
        <span className="text-3xl font-bold text-neutral-900 dark:text-neutral-100">{account.monthlyFee}</span>
  ...
  return (
    <Section background="light" className="py-12 lg:py-16">
      <Container size="xl">
        ...
        >
          <span className="inline-block px-3 py-1 text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/50 rounded-full mb-3">
            Account Options
          </span>
