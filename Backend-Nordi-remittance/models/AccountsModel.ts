...
const AccountLimitSchema = new Schema({
  wallet: { type: Schema.Types.ObjectId, ref: 'Wallets', required: true },
  limitType: { type: String, enum: ['daily', 'monthly', 'yearly', 'per_transaction'], required: true },
  category: { type: String, enum: ['withdrawal', 'transfer', 'payment', 'all'], required: true },
  amount: { type: Number, required: true },
  currency: { type: String, required: true },
...
export const AccountBalances = mongoose.model('AccountBalances', AccountBalanceSchema);
export const LedgerEntries = mongoose.model('LedgerEntries', LedgerEntrySchema);
export const AccountLimits = mongoose.model('AccountLimits', AccountLimitSchema);
export const AccountStatusHistories = mongoose.model('AccountStatusHistories', AccountStatusHistorySchema);
