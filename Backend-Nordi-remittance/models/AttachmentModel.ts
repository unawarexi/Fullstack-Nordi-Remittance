...
const AttachmentSchema: Schema = new Schema({
  ...
  storageProvider: { type: String, enum: ['s3', 'cloudinary', 'azure_blob', 'gcs', 'local'], required: true },
  category: { 
    type: String, 
    enum: ['kyc', 'proof_of_address', 'income_document', 'bank_statement', 'dispute_evidence', 'loan_document', 'tax_document', 'profile_picture', 'signature', 'other'], 
    required: true 
  },
...
AttachmentSchema.index({ attachmentId: 1 }, { unique: true });
