...
const LoanSchema = new Schema({
  ...
  loanType: { 
    type: String, 
    enum: ['personal', 'business', 'mortgage', 'auto', 'student', 'payday', 'line_of_credit'], 
    required: true 
  },
  ...
  disbursementMethod: { type: String, enum: ['bank_transfer', 'wallet', 'check'], required: true },
...
const CreditAssessmentSchema = new Schema({
  user: { type: String, ref: 'Users', required: true },
  loanApplication: { type: Schema.Types.ObjectId, ref: 'LoanApplications', required: true },
  ...
  employmentStability: { type: String, enum: ['excellent', 'good', 'fair', 'poor'], required: true },
  paymentHistory: { type: String, enum: ['excellent', 'good', 'fair', 'poor'], required: true },
  existingDebts: { type: Number, required: true },
  monthlyIncome: { type: Number, required: true },
  riskScore: { type: Number, required: true, min: 0, max: 100 },
  riskCategory: { type: String, enum: ['low', 'medium', 'high', 'very_high'], required: true },
  recommendation: { type: String, enum: ['approve', 'approve_with_conditions', 'reject'], required: true },
...
const RepaymentScheduleSchema = new Schema({
  loan: { type: Schema.Types.ObjectId, ref: 'Loans', required: true, unique: true },
  installments: [{
...
const CollateralSchema = new Schema({
  ...
  collateralType: { 
    type: String, 
    enum: ['property', 'vehicle', 'securities', 'equipment', 'inventory', 'other'], 
    required: true 
  },
  ...
  status: { type: String, enum: ['active', 'released', 'seized', 'liquidated'], default: 'active' },
...
LoanApplicationSchema.index({ applicationId: 1 }, { unique: true });
...
export const LoanApplications = mongoose.model('LoanApplications', LoanApplicationSchema);
export const CreditAssessments = mongoose.model('CreditAssessments', CreditAssessmentSchema);
export const RepaymentSchedules = mongoose.model('RepaymentSchedules', RepaymentScheduleSchema);
export const LoanRepayments = mongoose.model('LoanRepayments', LoanRepaymentSchema);
