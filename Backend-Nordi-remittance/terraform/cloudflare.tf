...
resource "cloudflare_zone_settings_override" "settings" {
  ...
  settings {
    ...
    # WebSocket support (critical for ride tracking)
    websockets = "on"
...
resource "cloudflare_ruleset" "waf_custom" {
  ...
  # Challenge suspicious payment requests
  rules {
    action      = "managed_challenge"
    expression  = "http.request.uri.path contains \"/api/v1/payments\" and not cf.bot_management.verified_bot"
    description = "Challenge unverified bots on payment endpoints"
    enabled     = true
  }
...
resource "cloudflare_ruleset" "rate_limiting" {
  ...
  # Payment rate limit: 5 requests per minute
  rules {
    action = "block"
    ratelimit {
      characteristics     = ["cf.colo.id", "ip.src"]
    ...
    expression  = "http.request.uri.path matches \"^/api/v1/payments\""
    description = "Payment endpoint rate limit"
    enabled     = true
  }

  # Driver location updates: 120 requests per minute (2/sec for GPS tracking)
  rules {
    action = "block"
    ratelimit {
      characteristics     = ["cf.colo.id", "ip.src"]
    ...
    expression  = "http.request.uri.path matches \"^/api/v1/drivers/.*/location$\""
    description = "Driver location update rate limit"
    enabled     = true
  }
}
...
resource "cloudflare_ruleset" "cache_rules" {
  ...
  # Never cache auth, rides, payments, or driver location
  rules {
    action = "set_cache_settings"
    action_parameters {
      cache = false
    }
    expression  = "http.request.uri.path matches \"^/api/v1/(auth|rides|payments|drivers/.*/location)\""
