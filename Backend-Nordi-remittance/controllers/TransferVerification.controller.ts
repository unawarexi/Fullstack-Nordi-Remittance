...
import {
  ...
  TransactionTaxes,
} from "../models/TransferVerificationModel.js";
...
export async function initiateSecureTransfer(
  ...
  try {
    ...
    const transaction = new Transactions({
      ...
      description:
        description ||
        `Secure transfer to ${recipient.firstName} ${recipient.lastName}`,
    ...
    sendCreated(
      ...
      {
        message:
          "Transfer initiated successfully. Please complete the 3-step security verification.",
        verification: {
          id: verification._id,
          ...
          totalSteps: 3,
          nextAction:
            "Request ISIN code by calling POST /transactions/secure-transfer/request-code",
        },
        transaction: {
          id: transaction._id,
          referenceNumber,
...
export async function requestVerificationCode(
  ...
  try {
    ...
    if (verification.expiresAt && new Date() > verification.expiresAt) {
      ...
      throw new ValidationError(
        "Verification has expired. Please initiate a new transfer.",
      );
    }
  ...
  try {
    ...
    if (codeData.attempts >= codeData.maxAttempts) {
      ...
      throw new ForbiddenError(
        "Maximum attempts exceeded. Verification failed.",
      );
    }
    ...
    if (nextStatus === "fully_verified") {
      ...
      sendSuccess(
        ...
        {
          message:
            "All verification steps completed! Your transfer has been processed.",
          verification: {
            id: verification._id,
...
async function completeVerifiedTransaction(
  ...
  // Get wallets
  const senderWallet = await Wallets.findById(transaction.wallet).session(
    session,
  );
  const recipientWallet = await Wallets.findById(
  ...
  const recipient = await Users.findById(
    verification.transactionDetails.recipientId,
  ).session(session);
