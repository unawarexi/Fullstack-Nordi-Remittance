...
export async function resetUserPassword(
  ...
  try {
    ...
    sendSuccess(
      res,
      null,
      "Password reset successfully. User notified via email.",
    );
  } catch (error) {
    next(error);
  }
...
export async function createOperationalTask(
  ...
  try {
    ...
    const { title, description, priority, assignedTo, dueDate, category } =
      req.body;
  ...
  try {
    ...
    if (!currentPassword || !newPassword || !otp) {
      throw new ValidationError(
        "Current password, new password, and OTP are required",
      );
    }
    ...
    const permissions = {
      ...
      financialOperations: [
        ...
        },
        {
          key: "canManageCards",
          label: "Manage Cards",
          description: "Manage card applications",
        },
      ],
      fraudSecurity: [
        {
