...
import { ConfirmationToken, LoginAttempt, SecurityEvent } from '../models/ConfirmModel.js';
import { hashPassword, comparePassword, generateSecureToken, generateOTP } from '../core/helpers/crypto.helper.js';
import { generateAuthTokens, verifyRefreshToken, generateVerificationToken, getAccessTokenCookieOptions, getRefreshTokenCookieOptions } from '../core/helpers/token.helper.js';
import { generateAccountNumber, generateWalletNumber, generateReferenceNumber } from '../core/helpers/generator.js';
import { validateRegistrationData, validateLoginData, isValidEmail, sanitizeString } from '../core/helpers/validation.helper.js';
import { sendSuccess, sendCreated, sendError, sendUnauthorized, sendNotFound } from '../core/helpers/response.helper.js';
import { UnauthorizedError, ValidationError, NotFoundError, AccountLockedError, RateLimitExceededError, UserAlreadyExistsError } from '../core/errors/AppError.js';
...
export async function register(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    if (!validation.isValid) {
      throw new ValidationError('Invalid registration data', { errors: validation.errors });
    }
 ...
 */
export async function registerFullKyc(req: Request, res: Response, next: NextFunction): Promise<void> {
  const session = await mongoose.startSession();
  session.startTransaction();
...
export async function login(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!validation.isValid) {
      throw new ValidationError('Invalid login credentials', { errors: validation.errors });
    }

    // Check Redis for locked account (rate limiting)
    const isLocked = await isLoginLocked(email);
    if (isLocked) {
      throw new AccountLockedError('Account is temporarily locked due to too many failed attempts. Try again later.');
    }
    ...
    if (!isPasswordValid) {
      ...
      if (remainingAttempts > 0) {
        throw new UnauthorizedError(`Invalid email or password. ${remainingAttempts} attempts remaining.`);
      } else {
        throw new AccountLockedError('Too many failed attempts. Account has been locked for 30 minutes.');
      }
    }
    ...
    if (user.status === 'deactivated') {
      throw new UnauthorizedError('Your account has been deactivated. Please contact support to reactivate.');
    }
    ...
    const wallet = await Wallets.findOne({ user: user._id, isPrimary: true })
...
export async function resendVerificationEmail(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (!user) {
      ...
      sendSuccess(res, null, 'If an account exists with this email, a verification link has been sent.');
      return;
    }
...
export async function changePassword(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    emitToUser(user._id.toString(), WS_EVENTS.PASSWORD_CHANGED, {
      type: 'password_changed',
      data: { message: 'Your password has been changed. Please login again on other devices.' },
      timestamp: new Date().toISOString(),
    });
