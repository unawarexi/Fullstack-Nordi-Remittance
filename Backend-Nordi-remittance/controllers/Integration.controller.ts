...
import { generateSecureToken, sha256Hash } from '../core/helpers/crypto.helper.js';
...
export async function getIntegrations(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');

    const integrations = await BankIntegrations.find()
      .select('-credentials.apiSecret -credentials.clientSecret -credentials.accessToken -credentials.privateKey')
      .sort({ bankName: 1 })
      .lean();
...
export async function getIntegrationById(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
...
export async function createIntegration(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
    ...
    if (!bankName || !bankCode || !integrationType) {
      throw new ValidationError('Bank name, code, and integration type are required');
    }
    ...
    const integration = new BankIntegrations({
      ...
      features: features || {},
      rateLimits: rateLimits || { requestsPerMinute: 60, requestsPerHour: 1000, requestsPerDay: 10000 },
      status: 'testing',
    });
...
export async function updateIntegration(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');

    const { id } = req.params;
    const { bankName, apiBaseUrl, apiVersion, credentials, features, rateLimits, status } = req.body;
    ...
    if (credentials) {
      integration.credentials = { ...(integration.credentials as any), ...credentials };
    }
    if (features) {
      integration.features = { ...(integration.features as any), ...features };
    }
    if (rateLimits) {
      integration.rateLimits = { ...(integration.rateLimits as any), ...rateLimits };
    }
...
export async function deleteIntegration(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
...
export async function testIntegration(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
...
export async function getIntegrationLogs(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
...
export async function getWebhooks(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
...
export async function updateWebhook(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const webhook = await WebhookSubscriptions.findOne({ _id: id, user: req.user.userId });
...
export async function deleteWebhook(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const webhook = await WebhookSubscriptions.findOneAndDelete({ _id: id, user: req.user.userId });
    if (!webhook) throw new NotFoundError('Webhook not found');
...
export async function regenerateWebhookSecret(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
...
export async function revokeApiKey(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
...
export async function getExternalAccounts(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
...
export async function linkExternalAccount(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
...
export async function unlinkExternalAccount(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');

    const { id } = req.params;

    const account = await ThirdPartyAccounts.findOne({ _id: id, user: req.user.userId });
...
export async function verifyExternalAccount(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.user) throw new UnauthorizedError('Authentication required');
