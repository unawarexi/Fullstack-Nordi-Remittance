...
export function isValidPassword(password: string): { isValid: boolean; errors: string[] } {
  ...
  if (password.length < constants.MIN_PASSWORD_LENGTH) {
    errors.push(`Password must be at least ${constants.MIN_PASSWORD_LENGTH} characters`);
  }
  
  if (password.length > constants.MAX_PASSWORD_LENGTH) {
    errors.push(`Password must not exceed ${constants.MAX_PASSWORD_LENGTH} characters`);
  }
...
export function validateRegistrationData(data: any): ValidationResult {
  ...
  const requiredFields = [
    ...
    'dateOfBirth', 'gender', 'nationality', 'countryOfResidence',
    'idType', 'idNumber', 'idExpiryDate', 'homeAddress', 'city',
    'stateProvince', 'zipCode', 'country', 'accountType', 'currency',
    'sourceOfIncome', 'monthlyIncomeRange', 'employmentStatus',
    'occupation', 'accountName', 'bankName', 'bankAddress', 'swiftBic',
    'securityQuestion', 'securityAnswer', 'addressDocType', 'taxIdentificationNumber'
  ];
  ...
  if (data.mobileNumber && !isValidPhone(data.mobileNumber)) {
    addError(errors, 'mobileNumber', 'Invalid phone number format', 'INVALID_FORMAT');
  }
  
  // Date of birth validation
  if (data.dateOfBirth && !isValidDateOfBirth(data.dateOfBirth)) {
    addError(errors, 'dateOfBirth', 'Invalid date of birth. Must be 18 years or older.', 'INVALID_DATE');
  }
  ...
  if (data.currency && !isValidCurrency(data.currency)) {
    addError(errors, 'currency', 'Invalid or unsupported currency', 'INVALID_CURRENCY');
  }
  ...
  if (data.initialDeposit !== undefined) {
    ...
    if (isNaN(deposit) || deposit < 0) {
      addError(errors, 'initialDeposit', 'Initial deposit must be a positive number', 'INVALID_AMOUNT');
    }
  }
  
  // IBAN or Routing number validation
  if (!data.ibanNumber && !data.routingNumber) {
    addError(errors, 'ibanNumber', 'Either IBAN or routing number is required', 'REQUIRED');
  }
  ...
  if (data.routingNumber && !isValidRoutingNumber(data.routingNumber)) {
    addError(errors, 'routingNumber', 'Invalid routing number', 'INVALID_FORMAT');
  }
  ...
  if (!data.agreeToTerms) {
    addError(errors, 'agreeToTerms', 'You must agree to the terms of service', 'REQUIRED');
  }
  
  if (!data.agreeToPrivacy) {
    addError(errors, 'agreeToPrivacy', 'You must agree to the privacy policy', 'REQUIRED');
  }
...
export function validateTransferData(data: any): ValidationResult {
  ...
  if (!data.toWalletId && !data.toAccountNumber) {
    addError(errors, 'recipient', 'Recipient wallet ID or account number is required', 'REQUIRED');
  }
  
  if (data.toAccountNumber) {
    if (!data.toBankName) {
      addError(errors, 'toBankName', 'Recipient bank name is required for external transfers', 'REQUIRED');
    }
    if (!data.toAccountName) {
      addError(errors, 'toAccountName', 'Recipient account name is required for external transfers', 'REQUIRED');
    }
  }
