...
import type { AuthenticatedRequest, UserRegistrationData } from '../../types/index.js';
import Users from '../users/users.model.js';
import { safeUploadToCloudinary, cleanupCloudinaryAssets } from '../../services/cloudinary.service';
import { Wallets } from '../accounts/accounts.model.js';
import { ConfirmationToken, LoginAttempt, SecurityEvent } from './confirm.model.js';
import {
  hashPassword,
  comparePassword,
  generateSecureToken,
...
import { generateAccountNumber, generateWalletNumber } from '../../core/helpers/generator.js';
import {
...
export async function register(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    ...
    // Kafka Event - User Registered (fire-and-forget — must not abort committed txn)
    const kafkaService = getKafkaService();
    kafkaService
      .publish(KafkaTopics.USER_REGISTERED, {
        userId: user._id.toString(),
        email: user.email,
        firstName: user.firstName,
        lastName: user.lastName,
      ...
      .catch((err) => console.error('Failed to publish Kafka registration event:', err));
  } catch (error) {
...
export async function registerFullKyc(
  ...
  // Track Cloudinary public_ids for cleanup on transaction failure
  ...
  try {
    ...
    // Check for existing user BEFORE starting expensive uploads/transactions
    const existingUser = await Users.findOne({
    ...
    // Hash password and security answer (CPU-bound, safe to do before txn)
    const hashedPassword = await hashPassword(password);
    const hashedSecurityAnswer = await hashPassword(securityAnswer.toLowerCase());

    // Generate account number
    const generatedAccountNumber = generateAccountNumber();

    // ── CLOUDINARY UPLOADS (OUTSIDE TRANSACTION) ──────────────────────────
    // External network I/O must NEVER happen inside a database transaction.
    // safeUploadToCloudinary returns null on failure instead of throwing,
    // so a Cloudinary timeout cannot crash the signup or abort the DB txn.
    ...
    // Upload all files concurrently for speed (each individually safe)
    const [profilePictureUrl, governmentIdUrl, proofOfAddressUrl, selfieWithIdUrl, signatureUrl] =
      await Promise.all([
        uploadFile(files['profilePicture']),
        uploadFile(files['governmentId']),
        uploadFile(files['proofOfAddress']),
        uploadFile(files['selfieWithId']),
        uploadFile(files['signature']),
      ]);

    // ── DATABASE TRANSACTION (no network I/O inside) ──────────────────────
    const session = await mongoose.startSession();
    session.startTransaction();
    ...
    try {
      ...
      if (duplicateCheck) {
        const field = duplicateCheck.email === email.toLowerCase() ? 'email' : 'mobile number';
        throw new UserAlreadyExistsError(field);
      }
      ...
      // Kafka events (fire-and-forget — NEVER block response or risk aborting committed txn)
      const kafkaService = getKafkaService();
      Promise.all([
      ...
      ]).catch((err) => console.error('Failed to publish Kafka events:', err));
    } catch (txnError) {
      await session.abortTransaction();
      session.endSession();

      // Clean up any Cloudinary assets that were uploaded before the DB failed
      cleanupCloudinaryAssets(uploadedPublicIds).catch((err) =>
        console.error('Failed to clean up Cloudinary assets after transaction rollback:', err),
      );
...
export async function login(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (user.lockUntil && (user.lockUntil as Date) > new Date()) {
      ...
      throw new AccountLockedError(`Account is locked. Try again in ${remainingTime} minutes.`);
    }
    ...
    if (!isPasswordValid) {
      ...
      const currentAttemptCount =
        (Array.isArray(user.loginAttempts) ? user.loginAttempts.length : 0) + 1;

      if (currentAttemptCount >= constants.MAX_LOGIN_ATTEMPTS || locked) {
        updates.lockUntil = new Date(Date.now() + constants.LOCKOUT_DURATION_MINUTES * 60 * 1000);
    ...
    await Promise.all([
      ...
      wallet ? cacheUserWallets(user._id.toString(), [wallet]) : Promise.resolve(),
    ]);
...
export async function verify2FA(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const user: any = await Users.findById(tokenDoc.userId).select('+twoFactorSecret');
    ...
    // Mark token as used
    await ConfirmationToken.updateOne({ _id: String(tokenDoc._id) }, { used: true });

    // If backup code was used, remove it
    if (backupCodes?.includes(code)) {
      await Users.updateOne({ _id: user._id as string }, { $pull: { backupCodes: code } });
    }
...
export async function refreshToken(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const user: any = await Users.findById(payload.userId).select('email role status');
...
export async function changePassword(
  ...
  try {
    ...
    emitToUser(user._id.toString(), WS_EVENTS.PASSWORD_CHANGED, {
      type: 'password_changed',
      data: {
        message: 'Your password has been changed. Please login again on other devices.',
      },
      timestamp: new Date().toISOString(),
    });
