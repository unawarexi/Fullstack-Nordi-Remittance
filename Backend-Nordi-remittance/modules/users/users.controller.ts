...
import { Wallets, AccountBalances, AccountLimits } from '../accounts/accounts.model.js';
import Transactions from '../transactions/transactions.model.js';
import { SecurityEvent, ConfirmationToken } from '../auth/confirm.model.js';
...
export async function disable2FA(
  ...
  try {
    ...
    const user: any = await Users.findById(req.user.userId).select('+password +twoFactorSecret');
    if (!user) {
      throw new NotFoundError('User not found');
    }
...
export async function getNotifications(
  ...
  try {
    ...
    sendPaginated(res, notifications, { page, limit, total }, 'Notifications retrieved');
  } catch (error) {
    next(error);
  }
}
...
export async function getUserById(
  ...
  try {
    ...
    const user: any = await Users.findById(id).select('-password -twoFactorSecret -backupCodes');
...
export async function updateUserKyc(
  ...
  try {
    ...
    const kycEmailStatus =
      kycStatus === 'expired' ? 'pending' : (kycStatus as 'pending' | 'approved' | 'rejected');
    ...
    await Promise.all([invalidateUserCache(id as string), invalidateKycCache(id as string)]);
...
export async function deleteUser(
  ...
  try {
    ...
    await invalidateUserCache(id as string);
    onUserWrite(id as string).catch(() => {});
    sendSuccess(res, { deletedId: id }, 'User deleted successfully');
  } catch (error) {
    next(error);
  }
}
...
export async function deleteAllUsers(
  ...
  try {
    ...
    sendSuccess(res, { count: result.deletedCount }, 'All users deleted successfully');
  } catch (error) {
    next(error);
  }
}
